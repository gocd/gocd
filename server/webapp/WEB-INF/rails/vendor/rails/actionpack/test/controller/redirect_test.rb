...
class RedirectController < ActionController::Base
  def simple_redirect
    redirect_to :action => "agents"
  end

  def redirect_with_status
    redirect_to({:action => "agents", :status => 301})
  end
  ...
  def module_redirect
    redirect_to :controller => 'module_test/module_redirect', :action => "agents"
  end
...
class RedirectTest < ActionController::TestCase
  ...
  def test_simple_redirect
    ...
    assert_equal "http://test.host/redirect/agents", redirect_to_url
  end
  ...
  def test_module_redirect
    get :module_redirect
    assert_response :redirect
    assert_redirected_to "http://test.host/module_test/module_redirect/agents"
  end

  def test_module_redirect_using_options
    get :module_redirect
    assert_response :redirect
    assert_redirected_to :controller => 'module_test/module_redirect', :action => 'agents'
  end
  ...
  def test_redirect_with_partial_params
    ...
    assert_deprecated(/test_redirect_with_partial_params/) do
      assert_redirected_to :action => 'agents'
    end
  end
...
module ModuleTest
  class ModuleRedirectController < ::RedirectController
    def module_redirect
      redirect_to :controller => '/redirect', :action => "agents"
    end
  end
  ...
  class ModuleRedirectTest < ActionController::TestCase
    ...
    def test_simple_redirect
      get :simple_redirect
      assert_response :redirect
      assert_equal "http://test.host/module_test/module_redirect/agents", redirect_to_url
    end
    ...
    def test_module_redirect_using_options
      get :module_redirect
      assert_response :redirect
      assert_redirected_to :controller => '/redirect', :action => "agents"
    end
  end
end
