...
module ApiV3
  module Config
    module TrackingTool
      class TrackingToolRepresenter < ApiV3::BaseRepresenter
        ...
        class << self
          def get_class(type)
            TRACKING_TOOL_TYPE_TO_CLASS_MAP[type] || (raise ApiV3::UnprocessableEntity, "Invalid Tracking Tool type '#{type}'. It has to be one of '#{TRACKING_TOOL_TYPE_TO_CLASS_MAP.keys.join(', ')}.'")
          end
        end
        ...
        def type
          TRACKING_TOOL_CLASS_TO_TYPE_MAP[tracking_tool.class] || (raise ApiV3::UnprocessableEntity, "Invalid Tracking Tool type '#{tracking_tool.class}'. It has to be one of '#{TRACKING_TOOL_CLASS_TO_TYPE_MAP.keys.join(', ')}.'")
        end
      end
    end
  end
