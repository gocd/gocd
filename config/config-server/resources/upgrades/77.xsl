...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:template match="/cruise/@schemaVersion">
        <xsl:attribute name="schemaVersion">77</xsl:attribute>
    </xsl:template>
    ...
    <xsl:template match="/cruise/pipelines/pipeline/stage/jobs/job/tasks/exec[@command]">
        <xsl:copy>
            <xsl:copy-of select="@*[name(.)!='command']"/>
            <xsl:attribute name="executable"><xsl:value-of select="@command"/></xsl:attribute>
        </xsl:copy>
    </xsl:template>
