...
@Component
public class ElasticProfileControllerV3 extends ApiController implements SparkSpringController, CrudController<ElasticProfile> {
    ...
    @Autowired
    public ElasticProfileControllerV3(ElasticProfileService elasticProfileService, ApiAuthenticationHelper apiAuthenticationHelper, EntityHashingService entityHashingService, ClusterProfilesService clusterProfilesService) {
        super(ApiVersion.v3);
    ...
    public String index(Request request, Response response) throws IOException {
        ...
        return writerForTopLevelObject(request, response, outputWriter -> ElasticProfilesRepresenter.toJSON(outputWriter, userSpecificElasticProfiles, this::canAdminister));
    }
    ...
    public String show(Request request, Response response) throws IOException {
        ...
        return writerForTopLevelObject(request, response, writer -> ElasticProfileRepresenter.toJSON(writer, elasticProfile, canAdminister(elasticProfile.getId(), elasticProfile.getClusterProfileId())));
    }
    ...
    public Consumer<OutputWriter> jsonWriter(ElasticProfile elasticAgentProfileProfile) {
        return writer -> ElasticProfileRepresenter.toJSON(writer, elasticAgentProfileProfile, canAdminister(elasticAgentProfileProfile.getId(), elasticAgentProfileProfile.getClusterProfileId()));
    }
    ...
    private boolean canAdminister(String profileId, String clusterId) {
        return apiAuthenticationHelper.doesUserHasPermissions(currentUsername(), SupportedAction.ADMINISTER, SupportedEntity.ELASTIC_AGENT_PROFILE, profileId, clusterId);
    }

    @FunctionalInterface
    public
    interface Function<ElasticProfileId, ClusterProfileId> {
        boolean apply(ElasticProfileId id, ClusterProfileId clusterId);
    }
}
