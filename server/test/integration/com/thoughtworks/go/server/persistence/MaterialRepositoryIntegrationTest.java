...
public class MaterialRepositoryIntegrationTest {
    ...
    @Test
    public void shouldPersistShallowCloneFlagForGitMaterial() throws Exception {
        GitMaterialConfig gitMaterialConfig = MaterialConfigsMother.gitMaterialConfig("gitUrl", "submoduleFolder", "branch", true);
        MaterialInstance materialInstance = new GitMaterial(gitMaterialConfig).createMaterialInstance();
        repo.saveOrUpdate(materialInstance);
        MaterialInstance persisted = repo.find(materialInstance.getId());
        assertThat(((GitMaterial) persisted.toOldMaterial("foo", "folder", "bar")).isShallowClone(), is(true));
    }
