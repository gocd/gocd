...
module ApiV4
  module Config
    class ArtifactRepresenter < ApiV4::BaseRepresenter
      ...
      class << self
        def get_class_for_artifact_type(type)
          ARTIFACT_TYPE_TO_ARTIFACT_CLASS_MAP[type] || (raise ApiV4::UnprocessableEntity, "Invalid Artifact type: '#{type}'. It has to be one of #{ARTIFACT_TYPE_TO_ARTIFACT_CLASS_MAP.keys.join(', ')}")
        end
      end
    end
  end
