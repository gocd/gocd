...
export abstract class BaseClusterProfileModal extends Modal {
  ...
  body() {
    ...
    const clusterProfileSupportedPlugins = _.filter(this.pluginInfos, (pluginInfo: PluginInfo<any>) => {
      const elasticAgentExtensionInfo = pluginInfo.extensionOfType(ExtensionType.ELASTIC_AGENTS) as ElasticAgentSettings;
      return elasticAgentExtensionInfo && elasticAgentExtensionInfo.supportsClusterProfiles;
    });

    const pluginList = _.map(clusterProfileSupportedPlugins, (pluginInfo: PluginInfo<any>) => {
      return {id: pluginInfo.id, text: pluginInfo.about.name};
    });

    const pluginInfo = this.pluginInfos.find(
      (pluginInfo) => pluginInfo.id === this.clusterProfile().pluginId());

    this.elasticAgentPluginInfo(pluginInfo || this.pluginInfos[0]);

    const elasticAgentSettings = this.elasticAgentPluginInfo()
                                     .extensionOfType(ExtensionType.ELASTIC_AGENTS) as ElasticAgentSettings;
    return (
      <div>
        <div>
          <FormHeader>
        ...
        <div>
          ...
          <div className={foundationClassNames(foundationStyles.foundationGridHax, foundationStyles.foundationFormHax)}>
            <div class="row collapse">
              <AngularPluginNew
                pluginInfoSettings={stream(elasticAgentSettings.clusterProfileSettings)}
...
export class EditClusterProfileModal extends BaseClusterProfileModal {
  ...
  constructor(clusterProfileId: string,
        ...
        result.do(
          () => {
            this.onSuccessfulSave(
              <span>
                      The cluster profile <em>{this.clusterProfile().id()}</em> was updated successfully!
                     </span>
            );
            this.close();
          },
  ...
  modalTitle() {
    return "Edit profile " + this.clusterProfileId;
  }
}
