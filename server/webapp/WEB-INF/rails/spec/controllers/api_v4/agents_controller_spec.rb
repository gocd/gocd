...
describe ApiV4::AgentsController do
  ...
  describe "delete" do
    describe "security" do
      ...
      it 'should not allow normal users, with security enabled' do
        login_as_user
        expect(controller).to disallow_action(:delete, :destroy, params: {uuid: @agent.getUuid()}).with(403, 'You are not authorized to perform this action.')
      end
    end
  ...
  describe "update" do
    describe "security" do
      ...
      it 'should not allow normal users, with security enabled' do
        login_as_user
        expect(controller).to disallow_action(:patch, :update, params: {uuid: @agent.getUuid(), hostname: 'some-hostname'}).with(403, 'You are not authorized to perform this action.')
      end
    end
