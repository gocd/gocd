...
public class MaterialRepository extends HibernateDaoSupport {
    ...
    private long modificationAfter(final long id, final MaterialInstance materialInstance) {
        BigInteger result = getHibernateTemplate().execute(session -> {
    ...
    Modification findLatestModification(final MaterialInstance expandedInstance) {
        ...
        synchronized (cacheKey) {
            Modification modification = getHibernateTemplate().execute(session -> {
                ...
                return (Modification) query.uniqueResult();
            });
    ...
    public Long getTotalModificationsFor(final MaterialInstance materialInstance) {
        ...
        if (totalCount == null || totalCount == 0) {
            synchronized (key) {
                ...
                if (totalCount == null || totalCount == 0) {
                    totalCount = (Long) getHibernateTemplate().execute(new HibernateCallback<Long>() {
    ...
    public Modifications getModificationsFor(final MaterialInstance materialInstance, final Pagination pagination) {
        ...
        if (modifications == null) {
            synchronized (key) {
                ...
                if (modifications == null) {
                    List<Modification> modificationsList = (List<Modification>) getHibernateTemplate().execute(new HibernateCallback<List<Modification>>() {
                        public List<Modification> doInHibernate(Session session) throws HibernateException {
    ...
    public Long latestModificationRunByPipeline(final CaseInsensitiveString pipelineName, final Material material) {
        ...
        if (modificationId == null) {
            synchronized (key) {
                ...
                if (modificationId == null) {
                    modificationId = getHibernateTemplate().execute(session -> {
