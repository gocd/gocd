...
describe("Agent Job State Transitions Modal", () => {
  ...
  it("should render job information", () => {
    expect(helper.byTestId("key-value-key-job")).toContainText("Job");
    expect(helper.byTestId("key-value-value-job")).toContainText("up42/3/up42_stage/1/up42_job");
  });

  it("should render wait time", () => {
    expect(helper.byTestId("key-value-key-wait-time")).toContainText("Wait Time");
    ...
    expect(helper.byTestId("key-value-value-wait-time")).toContainText("00:00:21");
    expect(helper.byTestId("key-value-value-wait-time")).toContainText(moment.utc(end.diff(start)).format("HH:mm:ss"));
  });

  it("should render building time", () => {
    expect(helper.byTestId("key-value-key-building-time")).toContainText("Building Time");
    ...
    expect(helper.byTestId("key-value-value-building-time")).toContainText("00:00:25");
    expect(helper.byTestId("key-value-value-building-time")).toContainText(moment.utc(end.diff(start)).format("HH:mm:ss"));
  });

  it("should render total time", () => {
    expect(helper.byTestId("key-value-key-total-time")).toContainText("Total Time");
    const start = moment(json.job_state_transitions[0].state_change_time, defaultFormat);
    const end   = moment(json.job_state_transitions[5].state_change_time, defaultFormat);

    expect(helper.byTestId("key-value-value-total-time")).toContainText("00:00:46");
    expect(helper.byTestId("key-value-value-total-time")).toContainText(moment.utc(end.diff(start)).format("HH:mm:ss"));
  });
