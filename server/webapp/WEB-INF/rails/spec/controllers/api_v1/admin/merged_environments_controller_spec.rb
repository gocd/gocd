...
describe ApiV1::Admin::MergedEnvironmentsController do
  ...
  describe "show" do
    ...
    describe "security" do
      it 'should allow anyone, with security disabled' do
        disable_security
        expect(controller).to allow_action(:get, :show, params: {
      ...
      it 'should disallow anonymous users, with security enabled' do
        enable_security
        login_as_anonymous
        expect(controller).to disallow_action(:get, :show, params: {environment_name: @environment_name}).with(403, 'You are not authorized to perform this action.')
      end
