...
import static com.thoughtworks.go.apiv4.configrepos.representers.RulesRepresenter.Unknown
  ...
  class toJSON {
    ...
    void shouldSerializeRulesWithErrors() {
      ...
      def json = toArrayString({
        RulesRepresenter.toJSON(it, rules)
      })
    ...
    void shouldSerializeEmptyRules() {
      def json = toArrayString({
        RulesRepresenter.toJSON(it, new Rules())
      })
