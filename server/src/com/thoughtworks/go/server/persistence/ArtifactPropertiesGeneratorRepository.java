 ...
 */
@Service
public class ArtifactPropertiesGeneratorRepository  {
    ...
    @Autowired
    public ArtifactPropertiesGeneratorRepository(SessionFactory sessionFactory, TransactionTemplate transactionTemplate) {
    ...
    public void save(ArtifactPropertiesGenerator artifactPropertiesGenerator) {
        if (artifactPropertiesGenerator != null) {
            hibernateTemplate.save(artifactPropertiesGenerator);
        }
    }

    public List<ArtifactPropertiesGenerator> findByBuildId(long buildId) {
        return hibernateTemplate.find(GET_ARTIFACT_PROPERTY_GENERATORS_BY_BUILD_ID, buildId);
    }
