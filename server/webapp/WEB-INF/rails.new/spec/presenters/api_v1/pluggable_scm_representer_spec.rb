...
describe ApiV1::Scms::PluggableScmRepresenter do
  ...
  it 'should render a pluggable scm material with hal representation' do
    ...
    expect(actual_json).to have_link(:doc).with_url('http://api.go.cd/#pluggable-scms')
  ...
  it 'should render configuration value with hal representation' do
    actual_json   = ApiV1::Scms::ConfigurationPropertyRepresenter.new(get_non_secure_property).to_hash(url_builder: UrlBuilder.new)
    expect(actual_json).to eq(get_plain_text_hash)
  end

  it 'should render secure configuration value with hal representation' do
    presenter   = ApiV1::Scms::ConfigurationPropertyRepresenter.new(get_secure_property)
    actual_json = presenter.to_hash(url_builder: UrlBuilder.new)
    expect(actual_json).to eq(get_secure_hash_with_encrypted_value)
  end

  it 'should convert from secure hash with encrypted value to Configuration Property' do
    configuration_property    = ConfigurationProperty.new
    presenter = ApiV1::Scms::ConfigurationPropertyRepresenter.new(configuration_property)
    presenter.from_hash(secure_hash_with_encrypted_value)
    expect(configuration_property).to eq(get_secure_property)
  end

  it 'should convert from secure hash with clear text value to Configuration Property' do
    configuration_property    = ConfigurationProperty.new
    presenter = ApiV1::Scms::ConfigurationPropertyRepresenter.new(configuration_property)
  ...
  it 'should deserialize an ambiguous encrypted variable (with both value and encrypted_value) to a variable with errors' do
    config    = ConfigurationProperty.new
    presenter = ApiV1::Scms::ConfigurationPropertyRepresenter.new(config)
    config = presenter.from_hash(key: 'some-key', secure: true, value: 'plainText', encrypted_value: 'encryptedText')
  ...
  it 'should deserialize an unambiguous encrypted variable (with either value or encrypted_value) to a variable without errors' do
    configuration_property    = ConfigurationProperty.new
    presenter = ApiV1::Scms::ConfigurationPropertyRepresenter.new(configuration_property)
    presenter.from_hash(key: 'some-key', secure: true, value: 'plainText')
    expect(configuration_property.errors).to be_empty

    config    = ConfigurationProperty.new
    presenter = ApiV1::Scms::ConfigurationPropertyRepresenter.new(config)
    presenter.from_hash(key: 'another-key', secure: true, encrypted_value: 'encryptedText')
