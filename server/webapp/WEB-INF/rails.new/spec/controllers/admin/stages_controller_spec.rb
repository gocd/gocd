  ...
  describe "action" do
    ...
    describe "index" do
      ...
      it "should assign templates" do
        ...
        expect(assigns[:template_list][0]).to eq(CaseInsensitiveString.new("foo"))
        expect(assigns[:template_list][1]).to eq(CaseInsensitiveString.new("template-name"))
      end
    ...
    describe "create" do
      ...
      it "should show error message when config save fails for reasons other than validations" do
        expect(controller).to receive(:task_view_service).twice.and_return(task_view_service = double("task_view_service"))
        expect(task_view_service).to receive(:taskInstanceFor).with("exec").and_return(ExecTask.new())
        expect(task_view_service).to receive(:getTaskViewModelsWith).with(ExecTask.new('ls','','work')).and_return(tvms = [TaskViewModel.new(AntTask.new(), "new", "erb"), TaskViewModel.new(NantTask.new(), "new", "erb")].to_java(TaskViewModel))
        stub_save_for_validation_error do |result, config, node|
