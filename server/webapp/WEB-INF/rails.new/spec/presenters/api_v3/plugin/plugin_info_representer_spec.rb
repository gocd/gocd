...
describe ApiV3::Plugin::PluginInfoRepresenter do
  it 'renders plugin_info with hal representation' do
    ...
    member_of_configuration = com.thoughtworks.go.server.ui.plugins.PluginConfiguration.new('memberOf', HashMap.new({required: true, secure: false, display_name: 'memberOf', display_order: '1'}), nil)
    view = com.thoughtworks.go.server.ui.plugins.PluginView.new('plugin_view_template')
    role_settings = PluggableInstanceSettings.new([member_of_configuration], view)

    about = GoPluginDescriptor::About.new('plugin_name', 'plugin_version', nil, nil, nil, nil)
    plugin_descriptor = GoPluginDescriptor.new('plugin_id', nil, about, nil, nil, nil)
    plugin_info = PluginInfo.new(plugin_descriptor, 'plugin_type', 'plugin_display_name', settings, role_settings, nil);
  ...
  it 'should render plugin_info in absence of pluggable_settings' do
    about = GoPluginDescriptor::About.new('plugin_name', 'plugin_version', nil, nil, nil, nil)
    plugin_descriptor = GoPluginDescriptor.new('plugin_id', 'plugin_version', about, nil, nil, nil)
    plugin_info = PluginInfo.new(plugin_descriptor, 'plugin_type', nil, nil, nil, nil);
  ...
  it 'should render image when available' do
    ...
    about = GoPluginDescriptor::About.new('plugin_name', 'plugin_version', nil, nil, nil, nil)
    plugin_descriptor = GoPluginDescriptor.new('plugin_id', nil, about, nil, nil, nil)
    plugin_info = PluginInfo.new(plugin_descriptor, 'plugin_type', nil, nil, nil, image);
