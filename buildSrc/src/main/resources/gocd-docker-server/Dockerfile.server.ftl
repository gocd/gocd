...
FROM docker.io/chainguard/bash:latest AS gocd-server-unzip
ARG TARGETARCH
ARG UID=1000
<#if useFromArtifact >
...
RUN curl --fail --location --silent --show-error "https://download.gocd.org/binaries/${goVersions.fullVersion}/generic/go-server-${goVersions.fullVersion}.zip" > /tmp/go-server-${goVersions.fullVersion}.zip && \
    ...
    mv -v /go-server-${goVersions.goVersion}/wrapper/wrapper-${distro.operatingSystemVariant.tanukiWrapperAlias}-$WRAPPERARCH /go-server/wrapper/ && \
    mv -v /go-server-${goVersions.goVersion}/wrapper/libwrapper-${distro.operatingSystemVariant.tanukiWrapperAlias}-$WRAPPERARCH.so /go-server/wrapper/ && \
