...
public class AgentRegistrationControllerTest {
    ...
    public void shouldRegisterWithProvidedAgentInformation() {
        when(agentService.hasAgent("blahAgent-uuid")).thenReturn(false);
        ...
        verify(agentService).requestRegistration(new Username("some-agent-login-name"), AgentRuntimeInfo.fromServer(new Agent("blahAgent-uuid", "blahAgent-host", request.getRemoteAddr()), false, "blah-location", 34567L, "osx"));
    }
    ...
    public void shouldAutoRegisterAgent() {
        ...
        when(agentService.hasAgent(uuid)).thenReturn(false);
        when(goConfigService.serverConfig()).thenReturn(serverConfig);
        ...
        verify(agentService).requestRegistration(new Username("some-agent-login-name"), AgentRuntimeInfo.fromServer(new Agent(uuid, "host", request.getRemoteAddr()), false, "location", 233232L, "osx"));
        verify(agentService).register(any(Agent.class), eq(""), eq(""), any(HttpOperationResult.class));
    }
    ...
    public void shouldGenerateToken() {
        ...
        when(agentService.hasAgent("uuid-from-agent")).thenReturn(false);
    ...
    public void shouldRejectGenerateTokenRequestIfAgentIsInConfig() {
        ...
        when(goConfigService.serverConfig()).thenReturn(serverConfig);
        when(agentService.findAgent("uuid-from-agent")).thenReturn(AgentInstanceMother.idle());
        when(agentService.hasAgent("uuid-from-agent")).thenReturn(true);
