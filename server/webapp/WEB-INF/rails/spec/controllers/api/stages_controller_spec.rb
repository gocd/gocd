...
describe Api::StagesController do
  ...
  describe "cancel" do
    before :each do
      @schedule_service = mock('schedule_service')
      controller.stub!(:schedule_service).and_return(@schedule_service)
    end
    it "should resolve route to cancel" do
      params_from(:post, "/api/stages/424242/cancel").should == {:controller => "api/stages", :action => "cancel", :no_layout=>true, :id => "424242"}
    end
  ...
  describe "cancel_stage_using_pipeline_stage_name" do
    ...
    it "should resolve route to cancel_stage_using_pipeline_stage_name" do
      params_from(:post, "/api/stages/blah_pipeline/blah_stage/cancel").should == {:controller => "api/stages",
