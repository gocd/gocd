...
import {ClusterProfile, ClusterProfileJSON, ClusterProfiles} from "models/cluster_profiles/cluster_profiles";

export class ClusterProfilesCRUD {
  private static API_VERSION_HEADER = ApiVersion.v1;

  static all() {
    return ApiRequestBuilder.GET(SparkRoutes.apiAdminAccessClusterProfilesPath(), this.API_VERSION_HEADER)
                            .then((result: ApiResult<string>) => result.map((body) => {
                                    return ClusterProfiles.fromJSON(JSON.parse(body));
                                  })
                            );
  }
