...
module ApiV3
  module Config
    module Materials
      class MaterialRepresenter < ApiV3::BaseRepresenter
        ...
        class << self
          def get_material_type(type)
            TYPE_TO_MATERIAL_MAP[type] or (raise ApiV3::UnprocessableEntity, "Invalid material type '#{type}'. It has to be one of '#{TYPE_TO_MATERIAL_MAP.keys.join(' ')}'")
          end
        end
