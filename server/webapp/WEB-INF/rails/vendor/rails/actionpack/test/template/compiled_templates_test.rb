...
class CompiledTemplatesTest < Test::Unit::TestCase
  ...
  def test_template_gets_compiled
    with_caching(true) do
      ...
      assert_equal "Hello world!", render(:file => "test/agents.erb")
  ...
  def test_template_gets_recompiled_when_using_different_keys_in_local_assigns
    with_caching(true) do
      assert_equal 0, @compiled_templates.instance_methods.size
      assert_equal "Hello world!", render(:file => "test/agents.erb")
      assert_equal "Hello world!", render(:file => "test/agents.erb", :locals => {:foo => "bar"})
  ...
  def test_template_changes_are_not_reflected_with_cached_template_loading
    with_caching(true) do
      with_reloading(false) do
        assert_equal "Hello world!", render(:file => "test/agents.erb")
        modify_template "test/agents.erb", "Goodbye world!" do
          assert_equal "Hello world!", render(:file => "test/agents.erb")
        end
        assert_equal "Hello world!", render(:file => "test/agents.erb")
      end
  ...
  def test_template_changes_are_reflected_without_cached_template_loading
    with_caching(true) do
      with_reloading(true) do
        assert_equal "Hello world!", render(:file => "test/agents.erb")
        modify_template "test/agents.erb", "Goodbye world!" do
          assert_equal "Goodbye world!", render(:file => "test/agents.erb")
        end
        assert_equal "Hello world!", render(:file => "test/agents.erb")
      end
    end
  ...
  def test_template_becomes_missing_if_deleted_without_cached_template_loading
    with_reloading(true) do
      assert_equal 'Hello world!', render(:file => 'test/agents.erb')
      delete_template 'test/agents.erb' do
        assert_raise(ActionView::MissingTemplate) { render(:file => 'test/agents.erb') }
      end
      assert_equal 'Hello world!', render(:file => 'test/agents.erb')
    end
  end
  ...
  def test_swapping_template_handler_is_working_without_cached_template_loading
    with_reloading(true) do
      assert_equal 'Hello world!', render(:file => 'test/agents')
      delete_template 'test/agents.erb' do
        rename_template 'test/hello_world_from_rxml.builder', 'test/agents.builder' do
          assert_equal "<html>\n  <p>Hello</p>\n</html>\n", render(:file => 'test/agents')
        end
      end
      assert_equal 'Hello world!', render(:file => 'test/agents')
    end
  ...
  def test_adding_localized_template_will_take_precedence_without_cached_template_loading
    with_reloading(true) do
      assert_equal 'Hello world!', render(:file => 'test/agents')
      rename_template 'test/agents.da.html.erb', 'test/agents.en.html.erb' do
        assert_equal 'Hey verden', render(:file => 'test/agents')
      end
    end
  end
  ...
  def test_deleting_localized_template_will_fall_back_to_non_localized_template_without_cached_template_loading
    with_reloading(true) do
      rename_template 'test/agents.da.html.erb', 'test/agents.en.html.erb' do
        assert_equal 'Hey verden', render(:file => 'test/agents')
        delete_template 'test/agents.en.html.erb' do
          assert_equal 'Hello world!', render(:file => 'test/agents')
        end
        assert_equal 'Hey verden', render(:file => 'test/agents')
      end
  ...
  def test_parallel_reloadable_view_paths_are_working
    with_reloading(true) do
      ...
      modify_template 'test/agents.erb', 'Goodbye world!' do
        assert_equal 'Goodbye world!', render(:file => 'test/agents')
        modify_template 'test/agents.erb', 'So long, world!' do
          with_view_paths(view_paths_copy, new_reloadable_view_paths) do
            assert_equal 'So long, world!', render(:file => 'test/agents')
          end
          assert_equal 'So long, world!', render(:file => 'test/agents')
        end
      end
