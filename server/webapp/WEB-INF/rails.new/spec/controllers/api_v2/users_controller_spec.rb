...
describe ApiV2::UsersController do
  ...
  describe :destroy_disabled_users do
    describe :security do
      it 'should allow anyone, with security disabled' do
        disable_security
        expect(controller).to allow_action(:delete, :destroy_disabled_users)
      end

      it 'should disallow anonymous users, with security enabled' do
        enable_security
        login_as_anonymous
        expect(controller).to disallow_action(:delete, :destroy_disabled_users).with(401, 'You are not authorized to perform this action.')
      end
    ...
    describe :admin do
      ...
      it 'should allow for deleting disabled users' do
        login_as_admin

        @user_service.should_receive(:deleteAllDisabledUsers).with(an_instance_of(HttpLocalizedOperationResult)) do |result|
          result.setMessage(LocalizedMessage.string("RESOURCES_DELETE_SUCCESSFUL", "user(s)", "[foo]"))
        end

        delete_with_api_header :destroy_disabled_users
        expect(response).to be_ok
        expect(response).to have_api_message_response(200, "The user(s) '[foo]' were deleted successfully.")
      end
    end

    describe :route do
      describe :with_header do
        it 'should route to destroy_disabled_users action of users controller' do
          expect(:delete => 'api/users').to route_to(action: 'destroy_disabled_users', controller: 'api_v2/users')
        end
      end
      describe :without_header do
        before :each do
        ...
        it 'should not route to show action of users controller without header' do
          expect(:delete => 'api/users').to_not route_to(action: 'destroy_disabled_users', controller: 'api_v2/users')
