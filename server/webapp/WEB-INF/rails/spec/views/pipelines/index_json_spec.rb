...
describe "/pipelines/index.json.erb" do
  ...
  describe "caching" do
    it "should cache pipeline partials of different pipelines separately" do
      ...
      key_proc = proc { |pipeline_group|
        [
            ["views",ViewCacheKey.new.forPipelineModelBox(pipeline_group.getPipelineModels().get(0))].join('/'),
            {:subkey => "pipeline_json_#{pipelines_dom_id(pipeline_group.getName())}", :skip_digest => true}
        ]
      }
