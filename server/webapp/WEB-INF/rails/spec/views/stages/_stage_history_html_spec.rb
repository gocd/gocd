...
describe "_stage_history.html.erb" do
  ...
  before(:each) do
    ...
    allow(view).to receive(:stage_detail_tab_path_for).with(instance_of(Hash)) do |params|
  ...
  it "should show config changed as a link when config has changed between top of this page and bottom of previous page" do
    ...
    expect(top_of_this_page).to receive(:getState).and_return(StageState::Building)
