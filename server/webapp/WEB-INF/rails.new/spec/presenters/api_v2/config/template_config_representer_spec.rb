...
describe ApiV2::Config::TemplateConfigRepresenter do
  before :each do
  ...
  it 'should render a template with hal representation' do
    actual_json = ApiV2::Config::TemplateConfigRepresenter.new(@template).to_hash(url_builder: UrlBuilder.new)
    expect(actual_json).to have_link(:self).with_url(UrlBuilder.new.apiv2_admin_template_url(template_name: @template.name.to_s))
  ...
  it 'should deserialize given json to PipelineTemplateConfig object' do
    ...
    ApiV2::Config::TemplateConfigRepresenter.new(deserialized_scm).from_hash(template_hash)
