...
public class ClusterProfilesRepresenter {
    public static void toJSON(OutputWriter writer, PluginProfiles<ClusterProfile> clusterProfiles) {
        writer.addLinks(
                ...
                .addChild("_embedded", embeddedWriter ->
                        embeddedWriter.addChildList("cluster_profiles", clustersWriter ->
                                clusterProfiles.forEach(cluster -> clustersWriter.addChild(clusterWriter -> ClusterProfileRepresenter.toJSON(clusterWriter, cluster)))));
    }
}
