  ...
  module Admin
    class TemplatesController < ApiV2::BaseController
      ...
      def index
        ...
        json = ApiV2::Config::TemplatesConfigRepresenter.new(templates).to_hash(url_builder: self)
        render DEFAULT_FORMAT => json
      end

      def show
        json = ApiV2::Config::TemplateConfigRepresenter.new(@template).to_hash(url_builder: self)
        render DEFAULT_FORMAT => json if stale?(etag: get_etag_for_template(@template))
      end

      def create
        result = HttpLocalizedOperationResult.new
        @template = ApiV2::Config::TemplateConfigRepresenter.new(PipelineTemplateConfig.new).from_hash(params[:template])
      ...
      def update
        result = HttpLocalizedOperationResult.new
        updated_template = ApiV2::Config::TemplateConfigRepresenter.new(PipelineTemplateConfig.new).from_hash(params[:template])
      ...
      def handle_create_or_update_response(result, updated_template)
        json = ApiV2::Config::TemplateConfigRepresenter.new(updated_template).to_hash(url_builder: self)
