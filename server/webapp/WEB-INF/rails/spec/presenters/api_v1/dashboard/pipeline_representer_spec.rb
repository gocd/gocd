...
describe ApiV1::Dashboard::PipelineRepresenter do
  ...
  it 'renders all pipeline groups with hal representation' do
    ...
    expect(actual_json).to have_link(:doc).with_url(CurrentGoCDVersion.api_docs_url '#pipelines')
    ...
    expect(actual_json).to have_link(:unpause).with_url('http://test.host/go/api/pipelines/pipeline_name/unpause')
    expect(actual_json).to have_link(:pause).with_url('http://test.host/go/api/pipelines/pipeline_name/pause')
