...
describe "admin/materials/index.html.erb" do
  ...
  describe "material name in the listing with the link to edit" do
    it "should display material name in the listing with the link to edit for svn" do
      ...
      expect(Capybara.string(response.body).all("td a[href='#'][class='material_name wrapped_word']")[0]['onclick']).to eq("Util.ajax_modal('/admin/pipelines/#{pipeline_name}/materials/svn/#{material_fingerprint}/edit', {readOnly: false, overlayClose: false, title: 'Edit Material - Subversion'}, function(text){return text})")
    end
    ...
    it "should display material name in the listing with the link to edit for package material" do
      ...
      expect(Capybara.string(response.body).all("td a[href='#'][class='material_name wrapped_word']")[1]['onclick']).to eq("Util.ajax_modal('/admin/pipelines/#{pipeline_name}/materials/#{material_type}/#{material_fingerprint}/edit', {readOnly: false, overlayClose: false, title: 'Edit Material - Package'}, function(text){return text})")
    end
  end
  ...
  describe "pluggable_scm_material" do
    ...
    it "should display material name in the listing with the link to edit for pluggable SCM material" do
      ...
      expect(Capybara.string(response.body).all("td a[href='#'][class='material_name wrapped_word']")[2]['onclick']).to eq("Util.ajax_modal('#{admin_pluggable_scm_edit_path(:pipeline_name => pipeline_name, :finger_print => material_fingerprint)}', {readOnly: false, overlayClose: false, title: 'Edit Material - Display Name'}, function(text){return text})")
    end
  end
