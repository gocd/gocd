...
abstract class ScmFields extends MithrilViewComponent<Attrs> {
  ...
  view(vnode: m.Vnode<Attrs>): m.Children {
    const materialAttributes = vnode.attrs.material.attributes() as ScmMaterialAttributes;
    return [
      this.requiredFields(materialAttributes),
      <TestConnection material={vnode.attrs.material}/>,
      <AdvancedSettings forceOpen={materialAttributes.errors().hasErrors("name") || materialAttributes.errors().hasErrors("destination")}>
        {this.extraFields(materialAttributes)}
        <TextField label={[
          "Alternate Checkout Path",
          " ",
          <Tooltip.Help size={TooltipSize.medium} content={DESTINATION_DIR_HELP_MESSAGE}/>
        ]} property={materialAttributes.destination} errorText={this.errs(materialAttributes, "destination")}/>
        <TextField label="Material Name" helpText={IDENTIFIER_FORMAT_HELP_MESSAGE}
                   placeholder="A human-friendly label for this material" property={materialAttributes.name}
                   errorText={this.errs(materialAttributes, "name")}/>
      </AdvancedSettings>
    ];
  }
...
export class P4Fields extends ScmFields {
  requiredFields(attrs: MaterialAttributes): m.Children {
    ...
    return [
      <TextField label="P4 [Protocol:][Host:]Port" property={mat.port} errorText={this.errs(attrs, "port")}
                 required={true}/>,
...
export class TfsFields extends ScmFields {
  requiredFields(attrs: MaterialAttributes): m.Children {
    ...
    return [
      ...
      <TextField label="Project Path" property={mat.projectPath} errorText={this.errs(attrs, "projectPath")}
                 required={true}/>,
