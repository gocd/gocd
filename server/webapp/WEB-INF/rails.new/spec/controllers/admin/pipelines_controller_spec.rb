...
describe Admin::PipelinesController do
  ...
  describe "new" do
    ...
    it "should have template list assigned" do
      ...
      expect(assigns[:template_list]).to eq(TemplatesConfig.new([@cruise_config.getTemplateByName(CaseInsensitiveString.new(template_name))].to_java(PipelineTemplateConfig)))
    end
  ...
  describe "create" do
    ...
    it "should throw up if pipeline name is empty and populate all states required for new action" do
      ...
      expect(task_view_service).to receive(:getViewModel).with(anything, anything).and_return(TaskViewModel.new(nil, nil, nil))
      expect(task_view_service).to receive(:getModelOfType).with(anything, anything).and_return(TaskViewModel.new(nil, nil, nil))
      expect(@security_service).to receive(:modifiableGroupsForUser).with(@user).and_return(["group1", "group2"])
      allow(@security_service).to receive(:hasViewOrOperatePermissionForPipeline).and_return(true)
    ...
    it "should validate pluggable tasks before create" do
      ...
      expect(task_view_service).to receive(:taskInstanceFor).with("pluggableTask").and_return(@new_task)
      expect(task_view_service).to receive(:getViewModel).with(@new_task, "new").and_return(TaskViewModel.new(nil, nil, nil))
      expect(task_view_service).to receive(:getModelOfType).with(anything, anything).and_return(TaskViewModel.new(nil, nil, nil))
      expect(@go_config_service).to receive(:getCurrentConfig).twice.and_return(Cloner.new().deepClone(@cruise_config))
      stub_save_for_validation_error do |result, cruise_config, pipeline|
        result.badRequest(LocalizedMessage.string("SAVE_FAILED"))
