...
public class GoConfigParallelGraphWalkerTest {
    ...
    public void shouldWalkCruiseConfigObjectsParallelly() {
        ...
        assertThat(rawCruiseConfig.pipelineConfigByName(cis("bad pipeline name")).errors())
            .isEqualTo(cruiseConfig.pipelineConfigByName(cis("bad pipeline name")).errors());
    }
    ...
    public void shouldHandlePipelinesWithTemplates() {
        ...
        cruiseConfig.getTemplates().add(
            new PipelineTemplateConfig(cis("template-1"), new StageConfig(cis("invalid stage name"), new JobConfigs(new JobConfig("job-1"))
            )));
        PipelineConfig pipelineWithTemplate = new PipelineConfig(cis("pipeline-with-template"), MaterialConfigsMother.defaultMaterialConfigs());
        pipelineWithTemplate.setTemplateName(cis("template-1"));
        cruiseConfig.getGroups().getFirst().add(pipelineWithTemplate);

        CruiseConfig rawCruiseConfig = GoConfigMother.deepClone(cruiseConfig);
        MagicalGoConfigXmlLoader.validate(cruiseConfig);
        Validatable.copyErrors(cruiseConfig, rawCruiseConfig);
        assertThat(rawCruiseConfig.pipelineConfigByName(cis("pipeline-with-template")).errors().isEmpty()).isTrue();
        assertThat(cruiseConfig.pipelineConfigByName(cis("pipeline-with-template")).getStage(cis("invalid stage name")).errors().isEmpty()).isFalse();

        assertThat(rawCruiseConfig.getTemplateByName(cis("template-1")).errors().isEmpty()).isTrue();
    }
    ...
    public void shouldAddErrorsToRawCruiseConfigWhenTemplateHasErrors() {
        CruiseConfig cruiseConfig = GoConfigMother.configWithPipelines("pipeline-1");
        cruiseConfig.getTemplates().add(
            new PipelineTemplateConfig(cis("invalid template name"), new StageConfig(cis("stage-1"), new JobConfigs(new JobConfig("job-1"))
            )));
        PipelineConfig pipelineWithTemplate = new PipelineConfig(cis("pipeline-with-template"), MaterialConfigsMother.defaultMaterialConfigs());
        pipelineWithTemplate.setTemplateName(cis("invalid template name"));
        cruiseConfig.getGroups().getFirst().add(pipelineWithTemplate);
        ...
        ConfigErrors templateErrors = rawCruiseConfig.getTemplateByName(cis("invalid template name")).errors();
