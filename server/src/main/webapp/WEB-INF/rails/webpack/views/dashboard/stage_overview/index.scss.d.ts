// This file is automatically generated by type-declarations-for-css-modules.ts during webpack compile. Please do not change this file!
export const agentCell: string;
export const alignLeft: string;
export const asc: string;
export const building: string;
export const buildingStage: string;
export const cancelOrRerunStageIcon: string;
export const cancelled: string;
export const cancelledByWrapper: string;
export const cancelledStage: string;
export const checkboxCell: string;
export const completed: string;
export const completing: string;
export const consoleLogIconCell: string;
export const countContainer: string;
export const countLabel: string;
export const countText: string;
export const desc: string;
export const dummyContainerAboveStageDetailsLink: string;
export const durationCell: string;
export const durationSeparator: string;
export const failed: string;
export const failedStage: string;
export const failingStage: string;
export const flexBox: string;
export const hidden: string;
export const inProgress: string;
export const jobCountAndRerunContainer: string;
export const jobCountContainer: string;
export const jobListContainer: string;
export const jobName: string;
export const jobRerunContainer: string;
export const jobResult: string;
export const mappings: string;
export const nameCell: string;
export const names: string;
export const passed: string;
export const passedStage: string;
export const pipelineActivityAlignLeft: string;
export const pipelineActivityAlignRight: string;
export const pipelineActivityClass: string;
export const preparing: string;
export const progressBarContainer: string;
export const progressBarTooltip: string;
export const rerunWrapper: string;
export const rotate: string;
export const scheduled: string;
export const sourceRoot: string;
export const sources: string;
export const sourcesContent: string;
export const spin: string;
export const stageCounterDropdownWrapper: string;
export const stageDetailsPageLink: string;
export const stageHeaderContainer: string;
export const stageName: string;
export const stageNameContainer: string;
export const stageNameTitle: string;
export const stageOperationButtonGroup: string;
export const stageOverviewContainer: string;
export const stageOverviewStatus: string;
export const stageSettings: string;
export const stageTriggerAndTimingContainer: string;
export const stateCell: string;
export const stateTransitionItem: string;
export const statusCell: string;
export const tableBody: string;
export const tableHeader: string;
export const tableRow: string;
export const tooltipItem: string;
export const tooltipKey: string;
export const tooltipKeyValuePair: string;
export const tooltipMessage: string;
export const transitionCircle: string;
export const transitionItemTooltipKey: string;
export const transitionLine: string;
export const triggeredByAndOn: string;
export const triggeredByContainer: string;
export const unknown: string;
export const unknownProperty: string;
export const unsorted: string;
export const uploadingArtifacts: string;
export const uploadingartifacts: string;
export const version: string;
export const vsmClass: string;
export const waiting: string;
