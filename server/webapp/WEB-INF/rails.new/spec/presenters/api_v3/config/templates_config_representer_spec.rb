...
describe ApiV3::Config::TemplatesConfigRepresenter do

  it 'should render links' do
    template_with_pipelines = ['template-name', ['pipeline1', 'pipeline2']]
    actual_json = ApiV3::Config::TemplatesConfigRepresenter.new([template_with_pipelines]).to_hash(url_builder: UrlBuilder.new)
    ...
    expect(actual_json.fetch(:_embedded)).to eq({templates: [ApiV3::Config::TemplateSummaryRepresenter.new(template_with_pipelines).to_hash(url_builder: UrlBuilder.new)]})
  end
end
