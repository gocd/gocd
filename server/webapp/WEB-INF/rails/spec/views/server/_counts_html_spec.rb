...
describe "/server/_counts.json.erb" do
  ...
  it "should not render errors when error count is zero" do
    ...
    response.should_not have_tag('.messages .error_count', "Errors: 0")
  end
  ...
  it "should render warnings and errors if there are any in the header" do
    ...
    response.should have_tag('a .messages .error_count', "Errors: 2")
    response.should have_tag('a .messages .warning_count', "Warnings: 1")
    response.should have_tag('.messages', /Errors: 2 &\s+ Warnings: 1/)
    end
  ...
  it "should render only errors if there are only errors in the header" do
    ...
    response.should have_tag('a .messages .error_count', "Errors: 2")
    response.should_not have_tag('.messages .warning_count')
  end
  ...
  it "should render only warnings if there are only warnings in the header" do
    ...
    assigns[:current_server_health_states] = ServerHealthStates.new([first])

    render :partial => "server/counts.html.erb"

    response.should_not have_tag('.messages .error_count')
    response.should have_tag('a .messages .warning_count', "Warnings: 1")
  end
