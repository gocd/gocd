...
describe AgentAutocompleteController do
  ...
  describe :routes do
    ...
    it "should resolve the path" do
      params_from(:get, "/agents/filter_autocomplete/resource").should == {:controller => "agent_autocomplete", :action => 'resource'}
      agent_filter_autocomplete_path(:action => "os").should == "/agents/filter_autocomplete/os"
    end

    it "should accept only the defined actions" do
      params_from(:get, "/agents/filter_autocomplete/foo").should == {:controller => "application", :action => 'unresolved', :url => ["agents", "filter_autocomplete", "foo"]}
  ...
  describe :actions do
    ...
    before do
      ...
      @go_config_service.should_receive(:getCurrentConfig).and_return(new_config = CruiseConfig.new)
    ...
    describe :name do
      ...
      it "should return all unique agent hostnames" do
        ...
        get "name", :q => ""
        response.body.should == "dev-agent\ntest-linux-agent\ntest-win-agent"
      end
      ...
      it "should return all unique agent hostnames starting with given query string" do
        ...
        get "name", :q => "te"
        response.body.should == "test-linux-agent\ntest-win-agent"
      end
    end
    ...
    describe :ip do
      ...
      it "should return all agent ip addresses" do
        ...
        get "ip", :q => ""
        response.body.should == "10.11.12.13\n11.13.14.15"
      end
      ...
      it "should return all agent ip addresses starting with a given query string" do
        ...
        get "ip", :q => "11."
        response.body.should == "11.13.14.15"
      end
    end
