...
describe Admin::BackupController do
  ...
  describe :routes do
    ...
    it "should resolve the route to the backup admin ui page" do
      params_from(:get, "/admin/backup").should == {:controller => "admin/backup", :action => "index"}
      route_for(:controller => "admin/backup", :action => "index").should == "admin/backup"
      backup_server_path.should == "/admin/backup"
    end

    it "should resolve the route for the server backup" do
      params_from(:post, "/admin/backup").should == {:controller => "admin/backup", :action => "perform_backup"}
