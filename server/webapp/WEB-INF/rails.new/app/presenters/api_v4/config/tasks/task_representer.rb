...
module ApiV4
  module Config
    module Tasks
      class TaskRepresenter < ApiV4::BaseRepresenter
        ...
        class << self
          ...
          def task_class_for_type(task_type)
            TASK_TYPE_TO_TASK_CLASS_MAP[task_type] || (raise ApiV4::UnprocessableEntity, "Invalid task type '#{task_type}'. It has to be one of '#{TASK_TYPE_TO_TASK_CLASS_MAP.keys.join(', ')}.'")
          end
        end
      end
    end
