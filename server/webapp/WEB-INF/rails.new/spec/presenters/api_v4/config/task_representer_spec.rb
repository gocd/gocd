...
describe ApiV4::Config::Tasks::TaskRepresenter do
  ...
  shared_examples_for 'tasks' do
    ...
    describe :serialize do
      ...
      it 'should render task with hal representation with run_if' do
        ...
        presenter                                = ApiV4::Config::Tasks::TaskRepresenter.new(with_run_if(run_if_config, existing_task))
        actual_json                              = presenter.to_hash(url_builder: UrlBuilder.new)
        expected_task_hash                       = task_hash
      ...
      it 'should render task with hal representation with oncancel' do
        ...
        actual_json                                 = presenter.to_hash(url_builder: UrlBuilder.new)
        expected_task_hash                          = task_hash
        expected_task_hash[:attributes][:on_cancel] = ApiV4::Config::Tasks::OnCancelRepresenter.new(existing_task.getOnCancelConfig).to_hash(url_builder: UrlBuilder.new)
        expect(actual_json).to eq(expected_task_hash)
      end
    end
    ...
    describe :deserialize do
      it 'should convert hash to Task' do
        ...
        presenter = ApiV4::Config::Tasks::TaskRepresenter.new(new_task)
        presenter.from_hash(ApiV4::Config::Tasks::TaskRepresenter.new(existing_task).to_hash(url_builder: UrlBuilder.new))
      ...
      it 'should convert hash with run_if to Task with run_if' do
        ...
        presenter = ApiV4::Config::Tasks::TaskRepresenter.new(new_task)
        presenter.from_hash(ApiV4::Config::Tasks::TaskRepresenter.new(task_with_run_if).to_hash(url_builder: UrlBuilder.new))
  ...
  describe :ant do
    ...
    it 'should represent the ant task attributes as null if blank' do
      task = AntTask.new
      ApiV4::Config::Tasks::TaskRepresenter.new(task).from_hash(default_task_hash)
      actual_json = ApiV4::Config::Tasks::TaskRepresenter.new(task).to_hash(url_builder: UrlBuilder.new)
  ...
  it 'should raise error when de-serializing a type that does not exist' do
    expect do
      ApiV4::Config::Tasks::TaskRepresenter.from_hash({type: :foo})
