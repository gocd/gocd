...
class ElasticProfileServiceTest {
    ...
    void shouldReturnElasticProfileWithGivenIdWhenPresent() {
        ElasticConfig elasticConfig = new ElasticConfig();
        ElasticProfile elasticProfile = new ElasticProfile("ecs", "cd.go.elatic.ecs", "prod-cluster");
        elasticConfig.getProfiles().add(elasticProfile);
    ...
    void shouldAddElasticProfileToConfig() {
        ElasticProfile elasticProfile = new ElasticProfile("ldap", "cd.go.ldap", "prod-cluster");
    ...
    class GetJobsUsingElasticProfile {
        ...
        void setUp() {
            ...
            elasticConfig.getProfiles().add(new ElasticProfile("docker", "cd.go.docker", "prod-cluster"));
            elasticConfig.getProfiles().add(new ElasticProfile("ecs", "cd.go.ecs", "prod-cluster"));
            elasticConfig.getProfiles().add(new ElasticProfile("kubernetes", "cd.go.k8s", "prod-cluster"));
            when(goConfigService.getElasticConfig()).thenReturn(elasticConfig);
        }
