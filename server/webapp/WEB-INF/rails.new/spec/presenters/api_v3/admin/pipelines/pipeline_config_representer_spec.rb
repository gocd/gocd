...
describe ApiV3::Admin::Pipelines::PipelineConfigRepresenter do

  describe "serialize" do
    it 'renders a pipeline with hal representation' do
      presenter   = ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(get_pipeline_config)
    ...
    it 'should serialize pipeline with template' do
      ...
      presenter       = ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config)
      actual_json     = presenter.to_hash(url_builder: UrlBuilder.new)
      actual_json.delete(:_links)
    ...
    def pipeline_with_template_hash
      {
        ...
        materials:               pipeline_with_template.materialConfigs().collect { |j| ApiV3::Admin::Pipelines::Materials::MaterialRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) },
        stages:                  nil,
        tracking_tool:           nil,
        timer:                   nil
      }
  ...
  describe "deserialise" do
    ...
    it "should convert from minimal json to PipelineConfig" do
      ...
      ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash(pipeline_hash_basic)
    ...
    describe "pipeline_with_environment_varibales" do
      it "should convert pipeline hash with environment variables  to PipelineConfig" do
        ...
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ environment_variables: environment_variables })
      ...
      it "should convert pipeline hash with empty environment variables  to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ environment_variables: [] })
    ...
    describe "pipeline_with_parmas" do
      it "should convert pipeline hash with parameters to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ parameters: [{
                                                                                                 name:  "command",
                                                                                                 value: "echo"
                                                                                               }, {
                                                                                                 name:  "command",
      ...
      it "should convert pipeline hash with empty parmas to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ parameters: nil })
    ...
    describe "pipeline_with_materials" do
      it "should convert pipeline hash with materials  to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ materials: [
                                                                                               {
                                                                                                 type:       'git',
                                                                                                 attributes: {
      ...
      it "should convert pipeline hash with empty materials  to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ materials: nil })
      ...
      it 'should raise exception when passing invalid material type' do
        ...
        expect do
          ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash(hash)
        end.to raise_error(ApiV3::UnprocessableEntity, /Invalid material type 'bad-material-type'. It has to be one of/)
      end
    ...
    describe "pipeline_with_stages" do
      it "should convert pipeline hash with stages  to PipelineConfig" do
        ...
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ stages: stages })
      ...
      it "should convert pipeline hash with empty stages  to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ stages: nil })
    ...
    describe "pipeline_with_tracking_tool" do
      it "should convert pipeline hash with tracking tool  to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ tracking_tool: {
          type:       "generic",
          attributes: {
            url_pattern:  "link",
            regex: "regex"
      ...
      it 'should raise exception when passing invalid tracking tool type' do
        ...
        end.to raise_error(ApiV3::UnprocessableEntity, /Invalid Tracking Tool type 'bad-tracking-tool'. It has to be one of/)
      end
    ...
    it "should convert from full blown document to PipelineConfig" do
      ...
      ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash(pipeline_hash)
    ...
    describe "pipeline_with_timer" do
      it "should convert pipeline hash with timer  to PipelineConfig" do
        pipeline_config = PipelineConfig.new
        ApiV3::Admin::Pipelines::PipelineConfigRepresenter.new(pipeline_config).from_hash({ timer: { spec: "0 0 22 ? * MON-FRI", only_on_changes: true } })
  ...
  def pipeline_hash
    {
      ...
      name:                    "wunderbar",
      template:                nil,
      parameters:              get_pipeline_config.getParams().collect { |j| ApiV3::Admin::Pipelines::ParamRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) },
      environment_variables:   get_pipeline_config.variables().collect { |j| ApiV3::Shared::EnvironmentVariableRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) },
      materials:               get_pipeline_config.materialConfigs().collect { |j| ApiV3::Admin::Pipelines::Materials::MaterialRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) },
      stages:                  get_pipeline_config.getStages().collect { |j| ApiV3::Shared::Stages::StageRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) },
      tracking_tool:           ApiV3::Admin::Pipelines::TrackingTool::TrackingToolRepresenter.new(get_pipeline_config.getTrackingTool).to_hash(url_builder: UrlBuilder.new),
      timer:                   ApiV3::Admin::Pipelines::TimerRepresenter.new(get_pipeline_config.getTimer).to_hash(url_builder: UrlBuilder.new)
    }
  end
