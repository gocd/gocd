...
import com.thoughtworks.go.server.newsecurity.authentication.filters.AbstractAuthenticationFilter;
...
@Component
public class AssumeAnonymousUserFilter extends AbstractAuthenticationFilter {
    ...
    protected User attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
        if (securityService.isSecurityEnabled()) {
            return new User("anonymous", "", Arrays.asList(GoAuthority.ROLE_ANONYMOUS.asAuthority()));
        } else {
            return new User("anonymous", "", Arrays.asList(GoAuthority.ROLE_SUPERVISOR.asAuthority()));
        }
    }
