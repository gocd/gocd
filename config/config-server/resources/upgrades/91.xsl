...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="migratedContent">
    ...
    <xsl:for-each select="//security/passwordFile">
      <xsl:element name="authConfig">
        <xsl:attribute name="id">passwordFile</xsl:attribute>
    ...
    <xsl:for-each select="//security/ldap">
      <xsl:element name="authConfig">
        <xsl:attribute name="id">ldap</xsl:attribute>
        ...
        <xsl:element name="property">
          <xsl:element name="key">LoginAttribute</xsl:element>
          <xsl:element name="value">
  ...
  <xsl:template match="//ldap|//passwordFile" priority="4"/>
