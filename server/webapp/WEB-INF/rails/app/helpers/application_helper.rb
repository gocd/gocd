...
module ApplicationHelper
  ...
  def session_has key
    session[key] || (default_key?(key) && GO_MESSAGE_KEYS.inject(false) { |found, key| found || flash.has_key?(key) })
  end
  ...
  def select_button name, options
    ...
    content_tag(:button, button_content(name, tag(:img, :src => image_path(image_url))), button_options(options))
  end

  def header_select_button name, options
    options[:class] = add_class(options[:class], 'header_submit')
    options[:type] = "button"
    content_tag(:button, button_content(name), button_options(options))
  end
  ...
  def image_button name, options
    ...
    options[:type] = "submit"
    options[:title] = name
    content_tag(:button, content_tag(:span, ' ', :title=>name), button_options(options))
  end
  ...
  def button_content name, *other_content
    content_tag(:span, name.upcase + other_content.to_s)
  end
  ...
  def use_compressed_js?
    com.thoughtworks.go.util.SystemEnvironment.new().use_compressed_js()
  end
  ...
  def version
    File.readlines(RAILS_ROOT + "/../vm/admin/admin_version.txt.vm")
  end
