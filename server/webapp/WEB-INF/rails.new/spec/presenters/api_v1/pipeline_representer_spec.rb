...
describe ApiV1::PipelineRepresenter do
  ...
  it 'renders all pipeline groups with hal representation' do
    ...
    expect(actual_json).to have_link(:doc).with_url('http://www.go.cd/documentation/user/current/api/v1/pipeline_api.html')
  ...
  def expected_self_links
    [
      {content_type: ApiV1::BaseRepresenter::CONTENT_TYPE_API_V1, href: 'http://test.host/api/pipelines/pipeline_name/history'},
      {content_type: ApiV1::BaseRepresenter::CONTENT_TYPE_HTML, href: 'http://test.host/tab/pipeline/history/pipeline_name'}
    ]
  end

  def expected_embedded_pipeline(pipeline_model)
    ApiV1::PipelineInstanceRepresenter.new(pipeline_model).to_hash(url_builder: UrlBuilder.new)
  end
