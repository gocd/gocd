...
describe ApiV1::StageRepresenter do
  it 'renders stages and previous stage with hal representation' do
    ...
    presenter   = ApiV1::StageRepresenter.new(stage_presenter_args)
    actual_json = presenter.to_hash(url_builder: UrlBuilder.new)

    expect(actual_json).to have_links(:self, :doc)
    expect(actual_json).to have_link(:self).with_url(expected_self_links)
    expect(actual_json).to have_link(:doc).with_url('http://www.go.cd/documentation/user/current/api/v1/stages_api.html')

    actual_json.delete(:_links)
    expect(actual_json.delete(:previous_stage)).to eq(ApiV1::StageRepresenter.new(presenter.previous_stage).to_hash(url_builder: UrlBuilder.new))
    expect(actual_json).to eq({name: 'stage2', status: 'Unknown'})
  end
  ...
  it 'renders stages (without previous stage) with hal representation' do
    ...
    presenter   = ApiV1::StageRepresenter.new(stage_value)
    actual_json = presenter.to_hash(url_builder: UrlBuilder.new)
  ...
  def expected_self_links
    [
      {content_type: ApiV1::BaseRepresenter::CONTENT_TYPE_API_V1, href: 'http://test.host/api/stages/pipeline-name/stage2/instance/2/2'},
      {content_type: ApiV1::BaseRepresenter::CONTENT_TYPE_HTML, href: 'http://test.host/pipelines/pipeline-name/2/stage2/2'}
    ]
  end
