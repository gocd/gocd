...
public class MaterialNotifyControllerV2 extends ApiController implements SparkSpringController {
    ...
    public String svnNotify(Request req, Response res) throws IOException {
        ...
        Optional<String> repoUrl = jsonReader.optString("repository_url");
        Optional<String> uuid = jsonReader.optString("uuid");
        if (repoUrl.isPresent()) {
            return notifyWithRepoUrl(req, res, "svn");
        } else if (uuid.isPresent()) {
            return notify(req, res, "svn", "uuid", uuid.get());
        } else {
            throw haltBecauseOfReason("Request payload should contain repository_url or uuid");
        }
    }
