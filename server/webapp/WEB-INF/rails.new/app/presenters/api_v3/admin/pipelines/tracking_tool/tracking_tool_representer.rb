    ...
    module Pipelines
      module TrackingTool
        class TrackingToolRepresenter < BaseRepresenter
          ...
          def type
            TRACKING_TOOL_CLASS_TO_TYPE_MAP[tracking_tool.class] || (raise UnprocessableEntity, "Invalid Tracking Tool type '#{tracking_tool.class}'. It has to be one of '#{TRACKING_TOOL_CLASS_TO_TYPE_MAP.keys.join(', ')}.'")
          end
        end
      end
    end
