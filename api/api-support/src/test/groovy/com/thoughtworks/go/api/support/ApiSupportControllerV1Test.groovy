...
class ApiSupportControllerV1Test implements SecurityServiceTrait, ControllerTrait<ApiSupportController> {
  ...
  class Get {
    ...
    void 'should return agent json'() {
      loginAsAdmin()
      def res = [foo: "bar"]
      when(serverStatusService.asJsonCompatibleMap(any() as Username, any(HttpLocalizedOperationResult))).thenReturn(res)
    ...
    void 'should return error response'() {
      ...
      }).when(serverStatusService).asJsonCompatibleMap(eq(currentUsername()), any(HttpLocalizedOperationResult))
