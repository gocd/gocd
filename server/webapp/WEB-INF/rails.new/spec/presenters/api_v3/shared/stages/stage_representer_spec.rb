...
describe ApiV3::Shared::Stages::StageRepresenter do
    ...
    def stage_hash
      {
        ...
        jobs:                    get_stage_config.getJobs().collect { |j| ApiV3::Shared::Stages::JobRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) }
      }
    end
  ...
  describe "deserialize" do
    it 'should convert basic hash to StageConfig' do
      ...
      ApiV3::Shared::Stages::StageRepresenter.new(stage_config).from_hash({name:                    'stage1',
                                                                   fetch_materials:         true,
                                                                   clean_working_directory: false,
                                                                   never_cleanup_artifacts: false, })
    ...
    it 'should convert basic hash with Approval to StageConfig' do
      ...
      ApiV3::Shared::Stages::StageRepresenter.new(stage_config).from_hash({approval: {
                                                                    type:          "manual",
                                                                    authorization: {
                                                                      roles: ["role1", "role2"],
                                                                      users: ["user1", "user2"]
    ...
    it 'should convert basic hash with Environment variables to StageConfig' do
      ...
      ApiV3::Shared::Stages::StageRepresenter.new(stage_config).from_hash(environment_variables)
    ...
    it 'should convert basic hash with Jobs to StageConfig' do
      ...
      ApiV3::Shared::Stages::StageRepresenter.new(stage_config).from_hash({jobs: [job_hash]})
  ...
  it "should render errors" do
    ...
    presenter = ApiV3::Shared::Stages::StageRepresenter.new(stage_config)
    actual_json = presenter.to_hash(url_builder: UrlBuilder.new)
  ...
  def stage_hash_with_errors(stage_config)
    {
      ...
      jobs:                    stage_config.getJobs().collect { |j| ApiV3::Shared::Stages::JobRepresenter.new(j).to_hash(url_builder: UrlBuilder.new) },
      errors:                  {
