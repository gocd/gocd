...
public class EmailNotificationIntegerationTest {
    ...
    private void agentReportJobIsCompleted(String unitLinux) {
        ...
        producer.reportCompleting(new AgentRuntimeInfo(new AgentIdentifier("localhost", "127.0.0.1", job.getAgentUuid()), AgentRuntimeStatus.Idle, currentWorkingDirectory(), "cookie", false), job.getIdentifier(),
                JobResult.Failed);
        producer.reportCurrentStatus(new AgentRuntimeInfo(new AgentIdentifier("", "", job.getAgentUuid()), AgentRuntimeStatus.Idle, currentWorkingDirectory(), "cookie", false), job.getIdentifier(),
                JobState.Completed);
    }
}
