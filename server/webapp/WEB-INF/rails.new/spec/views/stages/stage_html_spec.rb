    ...
    describe "pipeline tab" do
      ...
      it "should render unsupported message if IE8" do
        ...
        Capybara.string(response.body).find("div.vsm_not_supported.notification").tap do |f|
          expect(f).to have_selector "p.information", :text => /^[\s\S]*Your browser is not supported. Please either upgrade your browser or use a different browser to view <a href='\/go\/help\/value_stream_map\.html' target='_blank'>Value Stream Map<\/a>.[\s\S]*$/
        end
      end
