...
class StageInstanceAPIModel
  attr_reader :id, :pipeline_name, :pipeline_counter, :name, :counter, :rerun_of_counter, :state, :result, :scheduled, :approval_type, :approved_by, :operate_permission, :can_run, :jobs

  def initialize(stage_instance_model)
    @id = stage_instance_model.getId() unless stage_instance_model.getId() == nil
    @pipeline_name = stage_instance_model.getPipelineName() unless stage_instance_model.getState().to_s == "Unknown" || stage_instance_model.getPipelineName() == nil
    @pipeline_counter = stage_instance_model.getPipelineCounter() unless stage_instance_model.getState().to_s == "Unknown" || stage_instance_model.getPipelineCounter() == nil
    @name = stage_instance_model.getName() unless stage_instance_model.getName() == nil
    @counter = stage_instance_model.getCounter() unless stage_instance_model.getState().to_s == "Unknown" || stage_instance_model.getCounter() == nil
    @rerun_of_counter = stage_instance_model.getRerunOfCounter() unless stage_instance_model.getRerunOfCounter() == nil
    @state = stage_instance_model.getState().to_s unless stage_instance_model.getState() == nil
