...
public class GeneratePropertyCommandExecutor implements BuildCommandExecutor {
    ...
    public boolean execute(BuildCommand command, BuildSession buildSession) {
        ...
        String indent = "             ";
        if (!file.exists()) {
            buildSession.println(PUBLISH_ERR, format("%sFailed to create property %s. File %s does not exist.", indent, propertyName, file.getAbsolutePath()));
            return true;
        }

        try {
            if (!XpathUtils.nodeExists(file, xpath)) {
                buildSession.println(PUBLISH_ERR, format("%sFailed to create property %s. Nothing matched xpath \"%s\" in the file: %s.", indent, propertyName, xpath, file.getAbsolutePath()));
            } else {
                String value = XpathUtils.evaluate(file, xpath);
                buildSession.getPublisher().setProperty(new Property(propertyName, value));

                buildSession.println(PUBLISH, format("%sProperty %s = %s created." + "\n", indent, propertyName, value));
            }
        } catch (Exception e) {
            ...
            buildSession.getPublisher().reportErrorMessage(PUBLISH_ERR, message, e);
        }
