...
describe AgentDetailsController do
  ...
  describe 'agent_details' do
    ...
    before :each do
      ...
      expect(controller).to receive(:agent_service).and_return(@agent_service = Object.new)
    end
    ...
    it "should show job run history" do
      expect(@agent_service).to receive(:findAgentViewModel).with(@uuid).and_return(@agent)
      expect(controller).to receive(:job_instance_service).and_return(@job_instance_service = Object.new)
