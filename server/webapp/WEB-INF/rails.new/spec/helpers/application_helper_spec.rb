...
describe ApplicationHelper do
  ...
  it "should convert urls to https" do
    should_receive(:system_environment).and_return(env = double('sys_env'))
    expect(env).to receive(:getSslServerPort).and_return(8154)
    expect(make_https("http://user:loser@google.com/hello?world=foo")).to eq("https://user:loser@google.com:8154/hello?world=foo")
  end
  ...
  describe "plugins" do
    it "should render the right plugin template" do
      ...
      expect(renderer).to receive(:render).with(view_model, :view => self, "view" => self).and_return("view")
      expect(render_pluggable_template(view_model)).to eq("view")
    end
    ...
    it "should render the right form plugin template" do
      ...
      expect(renderer).to receive(:render).with(view_model, {"formNameProvider" => form_name_provider, :view => self, "view" => self, "foo" => "bar" }).and_return("view")
      expect(render_pluggable_form_template(view_model, form_name_provider, "foo" => "bar")).to eq("view")
    end
