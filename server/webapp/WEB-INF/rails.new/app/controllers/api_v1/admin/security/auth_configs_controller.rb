...
module ApiV1
  module Admin
    module Security
      class AuthConfigsController < ::ApiV1::BaseController
        ...
        def verify_connection
          ...
          message = "There was an unexpected server error while checking the connection. Please try again later or check the server logs for more information."
          Rails.logger.fatal("#{message} #{e.backtrace.join("\n")}")
          render_message(message, :internal_server_error)
        end
        ...
        def load_entity_from_config
          security_auth_config_service.findProfile(params[:auth_config_id]) || (raise ApiV1::RecordNotFound)
        end
