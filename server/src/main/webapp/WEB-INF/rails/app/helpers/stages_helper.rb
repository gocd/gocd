...
module StagesHelper
  ...
  def stage_detail_path_for_identifier(identifier, options = {})
    stage_detail_tab_path_for(options.merge(:pipeline_name => identifier.getPipelineName(),
  ...
  def stage_detail_tab_path_for options, tab=nil
    tab ||= (options.delete(:action) || 'default')
  ...
  def stage_bar_options sim
    raw tag.tag_options(:class => "stage_bar #{sim.getState()}", :title => "%s (%s)" % [sim.getName(), sim.getState()])
  end
