...
describe 'environments/index.json.erb' do
  ...
  describe "caching" do
    it "should cache pipeline partials of different pipelines separately" do
      ...
      key_proc = proc { |environment| [
          ["views",ViewCacheKey.new.forEnvironmentPipelineBox(environment.getPipelineModels()[0])].join('/'),
          {:subkey => "environment_json_#{env_dom_id(environment.name())}"}
      ]}
