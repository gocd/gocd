...
class DashboardControllerV3Test implements SecurityServiceTrait, ControllerTrait<DashboardControllerV3> {
  ...
  class Dashboard {
    ...
    class Etag {
      ...
      void 'should account for current user when computing etag'() {
        ...
        pipelineGroup.addPipeline(com.thoughtworks.go.apiv3.dashboard.GoDashboardPipelineMother.dashboardPipeline('pipeline1'))
        pipelineGroup.addPipeline(com.thoughtworks.go.apiv3.dashboard.GoDashboardPipelineMother.dashboardPipeline('pipeline2'))
        when(pipelineSelectionsService.load((String) isNull(), any(Long.class))).thenReturn(pipelineSelections)
        when(goDashboardService.hasEverLoadedCurrentState()).thenReturn(true)
  ...
  private static GoDashboardEnvironment environment(String name) {
    ...
    environment.addPipeline(com.thoughtworks.go.apiv3.dashboard.GoDashboardPipelineMother.dashboardPipeline('pipeline1'))
    return environment
  }
