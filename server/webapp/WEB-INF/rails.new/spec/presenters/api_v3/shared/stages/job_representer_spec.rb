...
describe ApiV3::Shared::Stages::JobRepresenter do
  ...
  describe :serialize do
    ...
    it 'should serialize run_instance_count with value set to 10' do
      ...
      presenter   = ApiV3::Shared::Stages::JobRepresenter.new(job_config)
      actual_json = presenter.to_hash(url_builder: UrlBuilder.new)
  ...
  describe :deserialize do
    ...
    it 'should convert attribute run_instance_count with value `all` to Job' do
      job_config = JobConfig.new
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash(run_instance_count: 'all')
    ...
    it 'should convert attribute run_instance_count with value `nil` to Job' do
      job_config = JobConfig.new
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash(run_instance_count: nil)
    ...
    it 'should convert attribute run_instance_count with integer value `nil` to Job' do
      job_config = JobConfig.new
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash(run_instance_count: '10')
      expect(job_config.run_on_all_agents).to eq(false)
    ...
    it 'should convert basic hash with environment variable to Job' do
      ...
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({ environment_variables: environment_variables })
    ...
    it 'should convert basic hash with task to Job' do
      ...
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({ tasks: [task_hash] })
    ...
    it 'should  raise exception when invalid task type is passed' do
      expect do
        job_config = JobConfig.new
        ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({ tasks: [{ type: 'invalid' }] })
    ...
    it 'should convert basic hash with tabs to Job' do
      ...
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({ tabs: tab_hash })
    ...
    it 'should raise exception when invalid artifact type is passed' do
      expect do
        job_config = JobConfig.new
        ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({ artifacts: [{ type: 'invalid' }] })
    ...
    it 'should convert basic hash with properties to Job' do
      ...
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({ properties: properties })
    ...
    it 'should convert attribute elastic_profile_id to Job with elastic_profile_id' do
      job_config = JobConfig.new
      ApiV3::Shared::Stages::JobRepresenter.new(job_config).from_hash({elastic_profile_id: 'docker.unit-test'})
